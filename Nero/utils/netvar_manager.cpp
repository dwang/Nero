...
{
	auto sig = remote::find_pattern(FNV("client.dll"), "\xC1\xFA\x05\x83\xE1\x1F\xB8", "xxxxxxx");
	auto headclass_ppp = reinterpret_cast<void*>(sig + 23);
...
auto netvar_manager::dump_recursive(const char* base_class, remote::variable<sdk::RecvTable>* table_ptr, const std::uint16_t offset) -> void
{
	...
	for (auto i = 0; i < table.m_nProps; ++i)
	{
		...
		if (prop.m_RecvType == sdk::DPT_DataTable &&
			...
			prop.m_pDataTable->get().m_pNetTableName->get()[0] == 'D') // Skip shitty tables
		{
